...
parser.add_argument('--data', type=str, default='../../data/beijing/padding_zeros.csv', help='location of the data file')
...
parser.add_argument('--CNN_kernel', type=int, default=3, help='kernel size of CNN')
parser.add_argument('--input_length', type=int, default=24, help='number of input hour')
parser.add_argument('--AR', type=int, default=1, help='AR')
#parser.add_argument('--save', type=str, default='model/model.h5', help='path to save model')
args = parser.parse_args()

df = pd.read_csv(args.data)
df.fillna(0, inplace=True)
df = df.iloc[24:, :]
df = df[['DEWP', 'TEMP', 'PRES', 'Iws', 'Is', 'Ir', 'pm2.5']]
df = df.values
df_train = df[1 * 24: (1 + 1278) * 24]
df_valid = df[(1 + 1278) * 24: (1 + 1278 + 183) * 24]
df_test = df[(1 + 1278 + 183) * 24:]
#df_test = df[-2730:]
...
num_hr = 24 * 7 + args.input_length + 1
...
for i in range(7):
  memory_X_train[:, df_train.shape[1] * args.input_length * i: df_train.shape[1] * args.input_length * (i + 1)] = memory_X_train_tmp[:, df_train.shape[1] * 24 * i: df_train.shape[1] * 24 * i + df_train.shape[1] * args.input_length]
  memory_X_valid[:, df_valid.shape[1] * args.input_length * i: df_valid.shape[1] * args.input_length * (i + 1)] = memory_X_valid_tmp[:, df_valid.shape[1] * 24 * i: df_valid.shape[1] * 24 * i + df_valid.shape[1] * args.input_length]
  memory_X_test[:, df_test.shape[1] * args.input_length * i: df_test.shape[1] * args.input_length * (i + 1)] = memory_X_test_tmp[:, df_test.shape[1] * 24 * i: df_test.shape[1] * 24 * i + df_test.shape[1] * args.input_length]

input_X_train = train[:, df_train.shape[1] * 24 * 7: df_train.shape[1] * 24 * 7 + df_train.shape[1] * args.input_length]
input_X_valid = valid[:, df_valid.shape[1] * 24 * 7: df_valid.shape[1] * 24 * 7 + df_train.shape[1] * args.input_length]
input_X_test = test[:, df_test.shape[1] * 24 * 7: df_test.shape[1] * 24 * 7 + df_train.shape[1] * args.input_length]
y_train = train[:, -1]
y_valid = valid[:, -1]
y_test = test[:, -1]
...
input_X_train_AR = np.reshape(input_X_train[:, :, -1, :], (-1, args.input_length))
input_X_valid_AR = np.reshape(input_X_valid[:, :, -1, :], (-1, args.input_length))
input_X_test_AR = np.reshape(input_X_test[:, :, -1, :], (-1, args.input_length))
...
input_AR = Input((args.input_length,))
input_1_encoded_m = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_1)
input_2_encoded_m = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_2)
input_3_encoded_m = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_3)
input_4_encoded_m = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_4)
input_5_encoded_m = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_5)
input_6_encoded_m = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_6)
input_7_encoded_m = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_7)
...
question_encoded = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(question)
...
input_1_encoded_c = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_1)
input_2_encoded_c = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_2)
input_3_encoded_c = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_3)
input_4_encoded_c = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_4)
input_5_encoded_c = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_5)
input_6_encoded_c = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_6)
input_7_encoded_c = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, 7), padding = 'valid', activation = 'relu')(input_7)
...
if args.AR:
  AR_output = Dense(1)(input_AR)
  main_output = add([main_output, AR_output])
...
model = Model(inputs = [input_1, input_2, input_3, input_4, input_5, input_6, input_7, question, input_AR], outputs = [main_output])
model.compile(loss = 'mae', optimizer = opt, metrics = ['mse', 'mae', 'mape'])
filepath = 'model/memdnn_split_attn_inputlength_' + str(args.input_length) +'_cnn_' + str(args.CNN_unit) + '_cnnkernel_' + str(args.CNN_kernel) + '_gru_' + str(args.GRU_unit) + '.h5'
...
print(model.summary())
model.fit([memory_X_train_1, memory_X_train_2, memory_X_train_3, memory_X_train_4, memory_X_train_5, memory_X_train_6, memory_X_train_7, input_X_train, input_X_train_AR], [y_train], epochs = 100, batch_size = 128, validation_data = ([memory_X_test_1, memory_X_test_2, memory_X_test_3, memory_X_test_4, memory_X_test_5, memory_X_test_6, memory_X_test_7, input_X_test, input_X_test_AR], [y_test]), callbacks = callbacks_list)
model = load_model(filepath)
if args.AR:
  predictions = (model.predict([memory_X_test_1, memory_X_test_2, memory_X_test_3, memory_X_test_4, memory_X_test_5, memory_X_test_6, memory_X_test_7, input_X_test, input_X_test_AR])) * math.sqrt(scaler.var_[-1]) + scaler.mean_[-1]
else:
...
testing_mape = mape(y_test_, predictions)
print("testing mape is", testing_mape)
testing_mae = mae(y_test_, predictions)
testing_rmse = rmse(y_test_, predictions)
