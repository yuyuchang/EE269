...
filepath = './model/LSTNet_inputlength_' + str(args.input_length) + '_gru_' + str(args.GRU_unit) + '_cnnkernel_' + str(args.CNN_kernel) + '_cnn_' + str(args.CNN_unit) + '.h5'
checkpoint = ModelCheckpoint(filepath, monitor = 'val_loss', save_weights_only = False, save_best_only = True)
earlystop = EarlyStopping(monitor = 'val_loss', patience = 5)
callbacks_list = [checkpoint, earlystop]
print(model.summary())
model.fit(X_train, y_train, epochs = 200, batch_size = 1024, validation_data = ([X_valid], [y_valid]), callbacks = callbacks_list)
