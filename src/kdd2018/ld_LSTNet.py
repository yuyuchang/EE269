...
parser.add_argument('--CNN_unit', type=int, default=32)
parser.add_argument('--GRU_unit', type=int, default=32)
parser.add_argument('--skip', type=int, default=24)
parser.add_argument('--input_length', type=int, default=48)
...
X_train = np.reshape(X_train, (-1, args.input_length, X_train.shape[1], 1))
X_valid = np.reshape(X_valid, (-1, args.input_length, X_valid.shape[1], 1))
X_test = np.reshape(X_test, (-1, args.input_length, X_test.shape[1], 1))
...
x0 = Input((X_train.shape[1], X_train.shape[2], X_train.shape[3]))

x = Conv2D(args.CNN_unit, kernel_size = (args.CNN_kernel, X_train.shape[2]), padding='same', activation='relu')(x0)
x = Reshape((-1, args.CNN_unit))(x)
r = GRU(args.GRU_unit, activation = 'relu', dropout = 0.2, recurrent_dropout = 0.2, kernel_initializer = 'glorot_normal', return_sequences = False)(x)
x_skip = Reshape((args.skip, int(args.input_length / args.skip), -1))(x)
r_skip = TimeDistributed(GRU(args.GRU_unit, activation = 'relu', dropout = 0.2, recurrent_dropout = 0.2, kernel_initializer = 'glorot_normal', return_sequences = False))(x_skip)

print(r)
#r = Flatten()(r)
r_skip = Flatten()(r_skip)
x_f = concatenate([r, r_skip])
y = Dense(y_train.shape[1])(x_f)

opt = optimizers.RMSprop(lr = 0.0001)
model = Model(inputs = x0, outputs = y)
model.compile(loss = 'mae', optimizer = opt, metrics = ['mse', 'mae'])
filepath = './model/ld_LSTNet_inputlength_' + str(args.input_length) + '_gru_' + str(args.GRU_unit) + '_cnnkernel_' + str(args.CNN_kernel) + '_cnn_' + str(args.CNN_unit) + '.h5'
checkpoint = ModelCheckpoint(filepath, monitor = 'val_loss', save_weights_only = False, save_best_only = True)
earlystop = EarlyStopping(monitor = 'val_loss', patience = 5)
callbacks_list = [checkpoint, earlystop]
print(model.summary())
model.fit(X_train, y_train, epochs = 200, batch_size = 512, validation_data = ([X_valid], [y_valid]), callbacks = callbacks_list)
...
filename = 'LSTNet_inputlength_' + str(args.input_length) + '_gru_' + str(args.GRU_unit) + '_cnnkernel_' + str(args.CNN_kernel) + '_cnn_' + str(args.CNN_unit) + '.csv'
